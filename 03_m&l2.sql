...
-- insert into pb_mlm_vod 
  ...
  -- aggregate final
prep as ( SELECT 
  show_id , episode_id , idvisitor ,idvisit, watched_minutes , server_time
  FROM final 
  where show_id  not in  (select distinct show_id from main ) )
select sum(watched_minutes)
from prep
 
   -- order by watched_minutes